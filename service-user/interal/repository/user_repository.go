...
func (ur *UserRepository) FetchByEmail(ctx context.Context, email string) (*models.User, error) {
	u := &models.User{}
	sqlStatement := fmt.Sprintf("SELECT * FROM users WHERE email = '%s' AND deleted_at IS NULL LIMIT 1;", email)
...
func (ur *UserRepository) FetchByID(ctx context.Context, id uint32) (*models.User, error) {
	u := &models.User{}
	sqlStatement := "SELECT * FROM users WHERE id = ? AND deleted_at IS NULL LIMIT 1"
	rows, err := ur.db.QueryContext(ctx, sqlStatement, id)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (ur *UserRepository) FetchUsers(ctx context.Context, limit, page int32) ([]models.User, int, error) {
	...
	var totalPages, count int
	row := ur.db.QueryRowContext(ctx, "SELECT COUNT(*) FROM users WHERE deleted_at IS NULL")
	err = row.Scan(&count)
	if err != nil {
		return nil, 0, err
	}
